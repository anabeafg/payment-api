...
describe("PaymentService", () => {
  ...
  it("deve atualizar um pagamento", async () => {
    const payment = { id: "123", amount: 100, status: PaymentStatus.PENDING, createdDate: "2020", updatedDate: "2020" };
    mockedRead.mockResolvedValue([payment]);
    mockedWrite.mockResolvedValue(undefined);
  
    const updated = await service.update("123", {amount: 200,status: PaymentStatus.PENDING,dueDate: "2025-05-23T14:00:00.000Z"} );
  ...
  it("deve lançar uma exceção ao tentar atualizar um pagamento não existente", async () => {
    mockedRead.mockResolvedValue([]);
    await expect(service.update("id-inexistente", {amount: 200,status: PaymentStatus.PENDING,dueDate: "2025-05-23T14:00:00.000Z"})).rejects.toThrow("Pagamento não encontrado");
  });
  
  it("deve atualizar o status do pagamento", async () => {
    const payment = { id: "321", amount: 100, status: PaymentStatus.PENDING, createdDate: "", updatedDate: "" };
    mockedRead.mockResolvedValue([payment]);
    mockedWrite.mockResolvedValue(undefined);
